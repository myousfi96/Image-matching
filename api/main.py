...
def extract_text_embeddings(text: str) -> np.ndarray:
    ...
    try:
        ...
        # Use numpy backend to avoid requiring PyTorch
        encoded = tokenizer([text], padding="max_length", truncation=True, return_tensors="np")
        # Ensure data is int32 as expected by Triton model
        ...
        # Assuming the model returns [batch_size, sequence_length, embedding_dim]
        # and we take the embedding of the [CLS] token (first token)
...
async def search_by_text(request: TextSearchRequest):
    ...
    try:
        ...
        # Extract embeddings for the text query
...
async def get_logs(limit: int = 50):
    ...
    try:
        ...
        # Format timestamps and ObjectIds for JSON serialization
        for log in logs:
            if "timestamp" in log and hasattr(log["timestamp"], 'isoformat'):
                log["timestamp"] = log["timestamp"].isoformat()
            # Convert MongoMock/ObjectId to str
            if "_id" in log:
                log["_id"] = str(log["_id"])
        return logs
    except Exception as e:
