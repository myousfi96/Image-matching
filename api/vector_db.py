...
# Use absolute path to avoid discrepancies when scripts are run from different
# working directories. Default: project_root/.qdrant_data
_DEFAULT_PATH = Path(__file__).resolve().parent / ".qdrant_data"
STORAGE_PATH = os.getenv("QDRANT_PATH", str(_DEFAULT_PATH))

_client: QdrantClient | None = None  # Lazily initialised singleton
...
def upsert_product_embeddings(
    ...
    # Prepare points with image vectors (required) and optional text vectors
    points = []
...
def search_embeddings(query: np.ndarray, top_k: int = 5) -> List[qmodels.ScoredPoint]:
    ...
    # Search using image vector space
