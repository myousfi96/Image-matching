...
def _build_trt(
    ...
    cmd = [
        ...
        "--fp16",
        # "--int8",  # NOTE: INT8 calibration can be brittle, disabling for robustness
        # "--best",  # NOTE: Can increase memory usage and build time, disabling for robustness
        "--verbose", # Add verbose logging to diagnose build issues
        "--memPoolSize=workspace:4096", # Limit workspace memory to 4GB
    ]
    if min_shapes:
...
def quantize_dinov2() -> str:
    ...
    # Build TensorRT engine (will raise an exception on failure)
    _build_trt(onnx_path, engine_path)
    onnx_path.unlink()  # Clean up ONNX file after successful conversion
...
def quantize_bge() -> str:
    ...
    dynamic = {
        "input0": {1: "seq"},  # token seq dim is dynamic
        "input1": {1: "seq"},
    }
